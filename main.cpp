...
void pureOfdmaThroughput() {
	...
	for (int nStas = 0; nStas <= 100; nStas += step) {
		...
		result = simulate_wlan(BANDWIDTH_80MHz, PURE_UL_OFDMA, RU_SIZE_26_TONES, 0, nStas, MCS_6, 100);
		printf("%6.2fM   ", result.throughput);

		for (int nRARUs = 1; nRARUs <= 16; nRARUs *= 2) {
			result = simulate_wlan(BANDWIDTH_80MHz, PURE_UL_OFDMA, RU_SIZE_26_TONES, nRARUs, nStas, MCS_6, 100);
			printf("%6.2fM   ", result.throughput);
		}
